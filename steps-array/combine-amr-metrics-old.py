...
def main() -> int:
    ...
    spf_files_se = find_files(root, "*_singlem.se_spf.tsv")
    
    # Apply SingleM control
    if args.singlem == "auto":
        use_singlem = bool(spf_files_pe or spf_files_se)
    else:
    ...
    if require_singlem:
        if not spf_files_pe:
            ...
            return 2
        if not spf_files_se:
            eprint("ERROR: --singlem on was set but no *_singlem.se_spf.tsv files were found.")
            return 2
    ...
    eprint(f"Found SPF files:     {len(spf_files_se)} (mode={args.singlem})")
    ...
    # Merge SPF_se if enabled and present
    if spf_files_se:
        spf_rows = []
        for f in spf_files_se:
            sample = sample_from_spf_se(f)
            spf_rows.append(read_spf_file_se(f, sample))
        df_spf_se = pd.concat(spf_rows, ignore_index=True) if spf_rows else pd.DataFrame()
    
        df_amr = df_amr.merge(df_spf_se, how="left", left_on="Name", right_on="sample").drop(
            columns=["sample"], errors="ignore"
        )
    else:
        df_amr["bacterial_archaeal_bases_se"] = pd.NA
        df_amr["metagenome_size_se"] = pd.NA
    ...
    if spf_files_pe:
        ...
        prok_bases_se = pd.to_numeric(df_amr["bacterial_archaeal_bases_se"], errors="coerce").fillna(0)
        prok_bases_tot = prok_bases_pe + prok_bases_se
        
        total_bases_pe = pd.to_numeric(df_amr["metagenome_size_pe"], errors="coerce").fillna(0)
        total_bases_se = pd.to_numeric(df_amr["metagenome_size_se"], errors="coerce").fillna(0)
        total_bases_tot = total_bases_pe + total_bases_se
